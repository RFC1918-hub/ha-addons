...
export interface ManualFormatResponse {
  title: string;
  artist: string;
  content: ContentNode[];
  onsong: string;
...
export async function formatManual(
  title: string,
  artist: string,
  content: string
): Promise<ManualFormatResponse> {
  const res = await fetch('/api/manual/format', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ title, artist, content }),
  });
  return handleResponse<ManualFormatResponse>(res);
}
